...
// Status for a plain tracked date (like a driver's CDL or medical card
// expiration) — no recurring-period logic, just overdue/soon/ok/missing.
export function dateOnlyStatus(dateStr) {
  if (!dateStr) return 'setdate';
  const d = fixedDate(dateStr);
  const t = today0();
...
// Flags any driver whose CDL or medical card is overdue or due within 30
// days, across all clients. Used by Overview, the Trucking page, and the
// calendar so a driver expiration is never a surprise.
export function computeDriverAlerts(clients) {
  const alerts = [];
  clients.forEach((c) => {
    (c.driverRoster || []).forEach((d) => {
      const cdlStatus = dateOnlyStatus(d.cdlExpires);
      if (cdlStatus === 'overdue' || cdlStatus === 'soon') alerts.push({ client: c, driver: d, field: 'cdl', fieldLabel: 'CDL', due: d.cdlExpires, status: cdlStatus });
      const medStatus = dateOnlyStatus(d.medExpires);
      if (medStatus === 'overdue' || medStatus === 'soon') alerts.push({ client: c, driver: d, field: 'med', fieldLabel: 'Medical card', due: d.medExpires, status: medStatus });
    });
  });
  return alerts;
}
